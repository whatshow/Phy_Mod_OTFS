...
yDD1_mat = np.squeeze(matlab_data["yDD1"]);
yDD2_mat = np.squeeze(matlab_data["yDD2"]);
content1_mat = np.squeeze(matlab_data["content1"]);
content2_mat = np.squeeze(matlab_data["content2"]);
# CE
...
assert(np.sum(yDD1_diff)/M/N < 1e-13);
content1_diff = abs(content1 - content1_mat);
assert(np.sum(content1_diff, axis=None)/M/N < 1e-13);
assert(np.sum(abs(his_est1 - his_est1_mat), axis=None)< 1e-13);

yDD2, his_est2, lis_est2, kis_est2 = rg2_rx.demap(threshold=1e-10);
content2 = rg2_rx.getContent();
# check
yDD2_diff = abs(yDD2 - yDD2_mat);
assert(np.sum(yDD2_diff)/M/N < 1e-13);
content2_diff = abs(content2 - content2_mat);
assert(np.sum(content2_diff, axis=None)/M/N <1e-13);
assert(np.sum(abs(his_est2 - his_est2_mat), axis=None)< 1e-13);
