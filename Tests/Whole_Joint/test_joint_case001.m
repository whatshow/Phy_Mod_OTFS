...
%% simulations
H_NMSE = zeros(length(SNR_ps), 1);
H_NMSE2 = zeros(length(SNR_ps), 1);
for idx = 1:length(SNR_ps)
    ...
    parfor i_Fram = 1:N_Fram
        ...
        if isempty(his_est)
            tmp_H_NMSE(i_Fram) = 10*log10(HDD_norm2);
        else
            HDD_est = otfs.getChannel(his_est, lis_est, kis_est);
            tmp_H_NMSE(i_Fram) = 10*log10(sum(abs(HDD-HDD_est).^2, "all")/HDD_norm2);
        end
        if isempty(his_est2)
